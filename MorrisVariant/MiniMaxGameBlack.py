...
def maxMin(board, depth, finalPosition):
        maxEval = float('-inf')
        for boardPosition in hlpr.generateMovesMidGameEndGameBlack(board):
                val = minMaxBlack(boardPosition, depth - 1, finalPosition, False)
                if val[0] > maxEval:
                        maxEval = val[0]
...
def minMax(board, depth, finalPosition):
        minEval = float('inf')
        for position in hlpr.generateMovesMidGameEndGame(board):
